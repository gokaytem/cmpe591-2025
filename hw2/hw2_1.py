...
def optimize_model(policy_net, target_net, memory, optimizer, batch_size, gamma):
    ...
    states = torch.stack([torch.tensor(s, dtype=torch.float32).to(device) for s in batch[0] if s is not None])
    ...
    next_states = torch.stack([torch.tensor(s, dtype=torch.float32).to(device) for s in batch[3] if s is not None])
...
def train():
    ...
    if os.path.exists('target_net.pth'):
