...
class Agent():
    def __init__(self):
        self.model = VPG(obs_dim=6, act_dim=2, hl=[64, 64]).to(device)
        self.critic1 = VPG(obs_dim=6 + 2, act_dim=1, hl=[64, 64]).to(device)
        self.critic2 = VPG(obs_dim=6 + 2, act_dim=1, hl=[64, 64]).to(device)
