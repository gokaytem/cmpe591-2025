...
import environment
from agent_soc_04 import Agent

cudnn.benchmark = True  # enable cuDNN auto-tuning
torch.set_num_threads(os.cpu_count() // 4 - 1)  # allow multi-threaded CPU ops
...
if __name__ == "__main__":
    ...
    if checkpoint_files:
        ...
        try:
            rews = np.load(f"rews_{file_suffix}_{episode_num}.npy").tolist()
        ...
        try:
            agent.model.load_state_dict(torch.load("model_{file_suffix}.pt", map_location=device))
            rews = np.load("rews_{file_suffix}.npy").tolist()
    ...
    for i in range(start_episode, num_episodes):
        ...
        if (i + 1) % 100 == 0:
            torch.save(agent.model.state_dict(), f"model_{file_suffix}_{i+1}.pt")
            np.save(f"rews_{file_suffix}_{i+1}.npy", np.array(rews))

    # Save the final model and the training statistics
    torch.save(agent.model.state_dict(), "model_{file_suffix}.pt")
    np.save("rews_{file_suffix}.npy", np.array(rews))
